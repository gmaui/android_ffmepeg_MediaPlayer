...
public class FileExplorerAdapter extends BaseAdapter {
    ...
    private void setRow(ViewHolder holder, int position) {
        ...
        if (position == 0 && !isTop) {
            holder.icon.setImageDrawable(mContext.getResources().getDrawable(R.drawable.ic_menu_back, null));
        } else if (file.isDirectory()) {
            holder.icon.setImageDrawable(mContext.getResources().getDrawable(R.drawable.ic_menu_archive, null));
        } else {
            Drawable d = null;
            if (FileExplorerActivity.checkExtension(file)) {
                d = mContext.getResources().getDrawable(R.drawable.ic_menu_gallery, null);
            } else {
                d = mContext.getResources().getDrawable(R.drawable.ic_menu_block, null);
            }
